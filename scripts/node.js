...
export class Node {
  ...
  constructor(el, name = "", type = "default", tname = null, id = null) {
    ...
    this.sockets = {
      ...
      upright: new Socket(
        $(el).find(".node-connection-socket.upright")[0],
        this,
        "upright"
      ),
      upleft: new Socket(
        $(el).find(".node-connection-socket.upleft")[0],
        this,
        "upleft"
      ),
      left: new Socket(
        $(el).find(".node-connection-socket.left")[0],
        this,
        "left"
      ),
      leftup: new Socket(
        $(el).find(".node-connection-socket.leftup")[0],
        this,
        "leftup"
      ),
      leftdown: new Socket(
        $(el).find(".node-connection-socket.leftdown")[0],
        this,
        "leftdown"
      ),
      down: new Socket(
        $(el).find(".node-connection-socket.down")[0],
        this,
        "down"
      ),
      downright: new Socket(
        $(el).find(".node-connection-socket.downright")[0],
        this,
        "downright"
      ),
      downleft: new Socket(
        $(el).find(".node-connection-socket.downleft")[0],
        this,
        "downleft"
      ),
      right: new Socket(
        $(el).find(".node-connection-socket.right")[0],
        this,
        "right"
      ),
      rightup: new Socket(
        $(el).find(".node-connection-socket.rightup")[0],
        this,
        "rightup"
      ),
      rightdown: new Socket(
        $(el).find(".node-connection-socket.rightdown")[0],
        this,
        "rightdown"
      ),
    };
  ...
  removeElementRequest(){
    $.ajax({
        url: "/diagram/" + localStorage.getItem("diagramId") + "/operation/remove",
        method: "post",
        dataType: "json",
        data: {
          elementId: this.id.toString()
