...
export class Node{
    ...
    constructor(el, name = "", type="default",tname=null, id=null){
        if(id==null){
            this.id = Node.idC;
            Node.idC++;
        }
            ...
            this.id = id;
            Node.idC = Node.idC > this.id ? Node.idC : (this.id+1);
        }
    ...
    #mmraw(e){
        ...
        e.preventDefault()
        if((coords[0] > rect.x + rect.width - 20) && (coords[1] > rect.y + rect.height - 20)){
    ...
    static fromJSON(json){
        ...
        const templateName = json.templateName || Node.deductTemplate(tname)
        let node;
        switch(tname){
            case "default":{
                node = new RectNode($($(`#${templateName}`).html())[0], json.text, json.type,templateName,json.id)
            }
            break;
            case "rhombus":{
                node = new RhombusNode($($(`#${templateName}`).html())[0], json.text, json.type,templateName,json.id)
            }
            break;
            case "object":{
                node = new ObjectNode($($(`#${templateName}`).html())[0], json.text, json.type,templateName,json.id, json.content)
            }
            break;
            case "class":{
                node = new ClassNode($($(`#${templateName}`).html())[0], json.text, json.type,templateName,json.id, json.content1, json.content2)
            }
            break;

            default:{
                node = new Node($($(`#${templateName}`).html())[0], json.text, json.type,templateName,json.id)
            }
            break;
        }
...
class RectNode extends Node{
    constructor(el, name, type, tname, id){
        super(el, name, type, tname, id)
    ...
    update(){
        super.update()
        if(!this.svgArea){return}
        $(this.lines[0]).attr("x1", "0").attr("x2", this.width).attr("y1", "0").attr("y2", "0")
        $(this.lines[1]).attr("x1", this.width).attr("x2", this.width).attr("y1", "0").attr("y2", this.height)
        $(this.lines[2]).attr("x1", "0").attr("x2", "0").attr("y1", "0").attr("y2", this.height)
        $(this.lines[3]).attr("x1", "0").attr("x2", this.width).attr("y1", this.height).attr("y2", this.height)
    }
}
...
class RhombusNode extends Node{
    ...
    update(){
        super.update()
        if(!this.svgArea){return}
        $(this.lines[0]).attr("x1", "0").attr("x2", (this.width)/2).attr("y1", (this.height)/2).attr("y2", "0")
        $(this.lines[1]).attr("x1", (this.width)/2).attr("x2", this.width).attr("y1", "0").attr("y2", (this.height)/2)
        $(this.lines[2]).attr("x1", "0").attr("x2", (this.width)/2).attr("y1", (this.height)/2).attr("y2", this.height)
        $(this.lines[3]).attr("x1", (this.width)/2).attr("x2", this.width).attr("y1", this.height).attr("y2", (this.height)/2)
    }
}
class ObjectNode extends Node{
    constructor(el, name, type, tname, id, content){
        super(el, name, type, tname, id)
        
        this.svgArea = $(this.el).find("svg")
        
        this.svgArea.html(`<line></line><line></line><line></line><line></line><line></line>`)
        this.lines = this.svgArea.find("line")
    ...
    set content(v){this.textContent = v; this.update()}
    ...
    update(){
        ...
        $(this.lines[2]).attr("x1", "0").attr("x2", "0").attr("y1", "0").attr("y2", this.height)
        $(this.lines[3]).attr("x1", "0").attr("x2", this.width).attr("y1", this.height).attr("y2", this.height)
        $(this.lines[4]).attr("x1", "0").attr("x2", this.width).attr("y1", this.height).attr("y2", this.height)
    }
    toJSON(){
...
class ClassNode extends Node {
    constructor(el, name, type, tname, id, fieldTexts, methodTexts){
        super(el, name, type, tname, id)
        
        this.svgArea = $(this.el).find("svg")
        
        this.svgArea.html(`<rect fill="none" stroke="black" stroke-width="3"></rect><line></line><line></line>`)
        this.lines = this.svgArea.find("line")
